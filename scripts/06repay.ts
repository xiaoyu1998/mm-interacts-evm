...
async function main() {
    ...
    const paramsRepay: RepayUtils.RepayParamsStructOutput = {
        positionId: bigNumberify(1),
        tokenIndex: POOL_BASE,
        repayAmount: usdtAmount,
        repayOption: REPAYMENT_PARTIAL
    };
    const multicallArgs = [
        exchangeRouter.interface.encodeFunctionData("executeRepay", [paramsRepay]),
    ];
